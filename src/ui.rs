...
impl Plugin for UiPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(MenuPage::Main)
            .add_systems(OnExit(GameState::AssetLoading), main_menu)
            .add_systems(OnEnter(GameState::Menu), show_menu)
            .add_systems(OnExit(GameState::Menu), hide_menu)
            .add_systems(
                Update,
                button::text_button_system.run_if(not(in_state(GameState::AssetLoading))),
            )
            .add_systems(
                Update,
                (change_menu, audio_state).run_if(in_state(GameState::Menu)),
            );
    }
}
...
fn change_menu(menu: Res<MenuPage>, mut menu_pages: Query<(&MenuPage, &mut Visibility)>) {
    if menu.is_changed() {
    ...
    for (interaction, mut image) in &mut interaction_query {
        if interaction == &Interaction::Pressed {
            ...
            *image = UiImage::new(match settings.audio {
                AudioSettings::ON => images.box_checked.clone(),
                AudioSettings::OFF => images.box_unchecked.clone(),
            });
        }
    }
}
...
pub fn main_menu(mut commands: Commands, images: Res<ImageAssets>, fonts: Res<FontAssets>) {
    commands
        .spawn((
            NodeBundle {
                style: Style {
                        ...
                        .with_children(|parent| {
                            parent.spawn((
                                ButtonBundle {
                                    ...
                                    image: UiImage::new(images.box_checked.clone()),
                                    ..default()
                                },
