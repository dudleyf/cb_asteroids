use crate::{
    assets::ImageAssets, kenney_assets::KenneySpriteSheetAsset, ship::PlayerShipType,
...
fn laser_movement(mut lasers: Query<(&mut Transform, &Laser)>, time: Res<Time>) {
    for (mut transform, Laser(preexisting_movement_factor)) in &mut lasers {
        let laser_facing_direction = transform.rotation * Vec3::Y;
        let translation_delta = *preexisting_movement_factor
            + laser_facing_direction.xy() * 100.0 * time.delta_seconds();
...
fn player_movement_system(
    ...
    transform.rotate_z(
        rotation_factor * ship.base_ship_speed().rotation_degrees_per_sec * time.delta_seconds(),
    );

    // get the ship's forward vector by applying the
    // current rotation to the ships initial facing
    // vector
    let user_facing_direction = transform.rotation * Vec3::Y;
    ...
    if keyboard_input.pressed(KeyCode::ArrowUp) {
        movement_factor.0 = (movement_factor.0 + 0.01 * user_facing_direction.xy())
    ...
    let movement_distance =
        movement_factor.0 * ship.base_ship_speed().movement_meters_per_sec * time.delta_seconds();
