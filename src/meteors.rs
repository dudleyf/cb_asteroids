...
const METEOR_BASE_SPEED_BIG: f32 = 1.0;
const METEOR_BASE_SPEED_MEDIUM: f32 = 1.2;
const METEOR_BASE_SPEED_SMALL: f32 = 1.4;

impl MeteorBundle {
    pub fn big(transform: Transform, space_sheet: &KenneySpriteSheetAsset) -> MeteorBundle {
        ...
        MeteorBundle {
            ...
            linear_movement: LinearMovement {
                movement_factor: Vec2::new(x as f32, y as f32),
                movement_direction: Quat::from_rotation_z(rotation),
            },
            spin: Spin(1.3),
            wrapping: WrappingMovement,
        }
    ...
    pub fn medium(transform: Transform, space_sheet: &KenneySpriteSheetAsset) -> MeteorBundle {
        let mut rng = rand::thread_rng();
    ...
    pub fn small(transform: Transform, space_sheet: &KenneySpriteSheetAsset) -> MeteorBundle {
        let mut rng = rand::thread_rng();
...
fn sandbox_meteor_destroyed_event_handler(
    ...
    let Some(space_sheet) = sheets.get(&images.space_sheet) else {
    ...
    } in &mut events.read()
    {
        match destroyed_type {
            MeteorType::Big => {
                for _ in 0..2 {
                    ...
                    commands.spawn(MeteorBundle::medium(
                        Transform::from_xyz(
                            destroyed_at.translation.x + x as f32,
                            destroyed_at.translation.y + y as f32,
                            1.0,
                        ),
                        &space_sheet,
                    ));
            ...
            MeteorType::Small => {
                // do nothing
                let mut rng = rand::thread_rng();
                let x: i32 = rng.gen_range((-width as i32 / 2)..(width as i32 / 2));
                let y: i32 = rng.gen_range((-height as i32 / 2)..(height as i32 / 2));
                commands.spawn(MeteorBundle::big(
                    Transform::from_xyz(x as f32, y as f32, 1.),
                    &space_sheet,
                ));
            }
        }
