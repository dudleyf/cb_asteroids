...
impl Plugin for ChooseShipPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<ChooseShipEvent>()
            .add_systems(OnEnter(GameState::ChooseShip), choose_ship_menu)
            .add_systems(
                Update,
                choose_ship_button_system.run_if(in_state(GameState::ChooseShip)),
            )
            .add_systems(OnExit(GameState::ChooseShip), hide_ship_menu);
    }
}
...
pub fn hide_ship_menu(mut choose_ship_menu: Query<&mut Visibility, With<ChooseShipMenu>>) {
...
pub fn choose_ship_menu(
    ...
    mut choose_ship_menu: Query<&mut Visibility, With<ChooseShipMenu>>,
) {
            ...
            commands
                .spawn((
                    ButtonBundle {
                        ...
                        image: images.pattern_blueprint.clone().into(),
                        ..default()
                    },
...
pub fn choose_ship_button_system(
    mut commands: Commands,
    mut interaction_query: Query<(&Interaction, &PlayerShipType, &Transform), Changed<Interaction>>,
    settings: Res<GameSettings>,
    sounds: Res<AudioAssets>,
    mut next_state: ResMut<NextState<GameState>>,
    mut choose_ship_events: EventWriter<ChooseShipEvent>,
) {
    for (interaction, ship_type, transform) in &mut interaction_query {
        match *interaction {
            Interaction::Pressed => {
