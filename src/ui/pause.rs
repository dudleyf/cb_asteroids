...
impl Plugin for PausePlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            handle_pause_toggle.run_if(in_state(GameState::Playing)),
        )
        .add_systems(
            Update,
            (
                show_pause_menu
                    ...
                    .run_if(resource_equals(Pausable::Paused)),
                hide_pause_menu
                    .run_if(resource_changed::<Pausable>)
...
fn handle_pause_toggle(input: Res<ButtonInput<KeyCode>>, mut pausable: ResMut<Pausable>) {
...
pub fn paused() -> impl Condition<()> {
    IntoSystem::into_system(|paused: Res<Pausable>| *paused == Pausable::Paused)
}
...
fn hide_pause_menu(mut commands: Commands, menu: Res<PauseMenu>) {
    commands.entity(menu.0).despawn_recursive();
}

fn show_pause_menu(mut commands: Commands, images: Res<ImageAssets>) {
    ...
    let pause_menu_id = commands
        .spawn(NodeBundle {
            background_color: Color::rgba(0.95, 0.95, 1.0, 0.1).into(),
            style: Style {
